...
def parallel_kata_attn_fwd_kernel(
    ...
    M: tl.constexpr,  # PSD num_groups
    H: tl.constexpr,
    HQ: tl.constexpr,
    G: tl.constexpr,  # = HQ // H (GQA grouping)
    bQ: tl.constexpr,  # query-block rows  (autotuned)
    ...
    idx_h = idx_hq // G
    if VARLEN:
        T = tl.load(seq_lengths + idx_b)
        bos = tl.load(seq_starts + idx_b)
    else:
        T = seq_lengths
        bos = idx_b * T

    eos = min(T, (idx_q + 1) * bQ)
    num_k_blocks = tl.cdiv(eos, bK)
    q_start_blk = (idx_q * bQ) // bK  # first key block that overlaps this query block

    offs_q = idx_q * bQ + tl.arange(0, bQ)
    offs_k = tl.arange(0, bK)

    desc_q_g = tl.make_tensor_descriptor(
        q_addr + (bos * HQ + idx_hq) * d_k,
        shape=[T, d_k],
        strides=[HQ * d_k, 1],
        block_shape=[bQ, E],
    )
    desc_k_g = tl.make_tensor_descriptor(
    ...
    ptr_den = den_addr + bos * HQ + idx_hq + (idx_q * bQ + tl.arange(0, bQ)) * HQ

    b_o = tl.zeros([bQ, bV], dtype=tl.float32)
    b_den = tl.zeros([bQ], dtype=tl.float32)

    if HAS_DECAY:
        b_cq = tl.load(
            c_addr + bos * HQ + idx_hq + (idx_q * bQ + tl.arange(0, bQ)) * HQ,
            mask=offs_q < T,
        )
        ptr_decay = c_addr + bos * HQ + idx_hq + tl.arange(0, bK) * HQ

    # Phase 1: strictly earlier key blocks (no causal mask)
    for idx_s in range(0, q_start_blk):
        b_scores = tl.zeros([bQ, bK], dtype=tl.float32)

        for m_id in tl.static_range(M):
            q_g = desc_q_g.load([idx_q * bQ, m_id * E])
            k_g = desc_k_g.load([idx_s * bK, m_id * E]).T
        ...
        if HAS_DECAY:
            b_decay = tl.load(ptr_decay, mask=offs_k < T)
            b_scores = b_scores * tl.exp(tl.minimum(b_cq[:, None] - b_decay[None, :], 0.0))
            ptr_decay += bK * HQ

        offs_k += bK

        b_v = desc_v.load([idx_s * bK, idx_v * bV])
        b_den += tl.sum(b_scores, axis=1)
        b_o += tl.dot(b_scores.to(b_v.dtype), b_v)

    # Phase 2: diagonal / overlapping blocks (causal mask)
    for idx_s in range(q_start_blk, num_k_blocks):
        b_scores = tl.zeros([bQ, bK], dtype=tl.float32)
        for m_id in tl.static_range(M):
            q_g = desc_q_g.load([idx_q * bQ, m_id * E])
            k_g = desc_k_g.load([idx_s * bK, m_id * E]).T
        ...
        offs_k2 = idx_s * bK + tl.arange(0, bK)
        m_s = (offs_q[:, None] >= offs_k2[None, :]) & (offs_k2 < T)[None, :]
        b_scores = tl.where(m_s, b_scores, 0.0)

        if HAS_DECAY:
            b_cs = tl.load(
                c_addr + bos * HQ + idx_hq + offs_k2 * HQ, mask=offs_k2 < T
            )
    ...
    # Normalize
    b_acc_safe = tl.where(b_den > 0, b_den, 1.0)
    b_o = b_o / b_acc_safe[:, None]
    o_rows = idx_q * bQ + tl.arange(0, bQ)
    o_cols = idx_v * bV + tl.arange(0, bV)
    tl.store(
        o_addr + (bos * HQ + idx_hq) * d_v
        + o_rows[:, None] * (HQ * d_v) + o_cols[None, :],
        b_o.to(o_addr.dtype.element_ty), mask=o_rows[:, None] < T,
    )
    tl.store(ptr_den, b_den.to(den_addr.dtype.element_ty), mask=offs_q < T)
...
def parallel_kata_attn_sum_fwd_kernel(
    ...
    """Sum-PSD score: A[t,s] = Sum_{i,j} (scale * q_i[t] . k_j[s])^2.
    ...
    state than concat). Matches the math used by paper KATA-PSD-4
