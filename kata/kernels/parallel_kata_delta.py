...
@triton.jit
def _read(
    b_x, Gm, C: tl.constexpr, E: tl.constexpr, BV: tl.constexpr, TF: tl.constexpr
):
...
def spd_delta_chunk_fwd_kernel_M2(
    ...
    for i_c in range(0, tl.cdiv(T, C)):
        ...
        p_q0 = tl.make_block_ptr(
            q + (bos * H + i_h) * Kd, (T, Kd), (H * Kd, 1), (t0, 0), (C, E), (1, 0)
        )
        p_k0 = tl.make_block_ptr(
            k + (bos * H + i_h) * Kd, (T, Kd), (H * Kd, 1), (t0, 0), (C, E), (1, 0)
        )
        p_q1 = tl.make_block_ptr(
            q + (bos * H + i_h) * Kd, (T, Kd), (H * Kd, 1), (t0, E), (C, E), (1, 0)
        )
        p_k1 = tl.make_block_ptr(
            k + (bos * H + i_h) * Kd, (T, Kd), (H * Kd, 1), (t0, E), (C, E), (1, 0)
        )
        ...
        # intra grams
        qk0 = tl.dot(b_q0, tl.trans(b_k0), allow_tf32=TF)
        qk1 = tl.dot(b_q1, tl.trans(b_k1), allow_tf32=TF)
        kk0 = tl.dot(b_k0, tl.trans(b_k0), allow_tf32=TF)
        kk1 = tl.dot(b_k1, tl.trans(b_k1), allow_tf32=TF)
        Sqk = qk0 * qk0 + qk1 * qk1
        Skk = kk0 * kk0 + kk1 * kk1
...
def _prescale(x):
    n2 = (x * x).sum(-1)
    a = (n2 * n2).sum(-1).clamp_min(1e-12).pow(-0.25)
...
def spd_delta(q, k, v, beta, chunk_size=32, BV=16, prescale=True):
    ...
    spd_delta_chunk_fwd_kernel_M2[grid](
        q.reshape(B, T, H, M * E).contiguous(),
        k.reshape(B, T, H, M * E).contiguous(),
        v.contiguous(),
        beta.contiguous(),
        o,
        T,
...
def spd_delta_fast(q, k, v, beta, chunk_size=64, BV=16, prescale=True):
    ...
    o_norm, den = parallel_kata_attn_fwd_impl(
        q.reshape(B, T, H, M * E).bfloat16(),
        k.reshape(B, T, H, M * E).bfloat16(),
        U.bfloat16(),
        M,
        1.0,
        64,
