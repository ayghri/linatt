    ...
    for c in range(NC):
        ...
        while L < C:
            ...
            Tm = Tm - tl.where(
                mL,
                tl.dot(
                    tl.dot(Tm, tl.where(mL, N, 0.0), allow_tf32=True),
                    Tm,
                    allow_tf32=True,
                ),
                0.0,
        ...
        for dv0 in range(0, DV, DVB):
            ...
            if WRITE_U:
                tl.store(
                    Uout + bv + rowd,
                    U.to(Uout.dtype.element_ty),
                    mask=(c0 + o_row)[:, None] < T,
                )
...
def spd_delta_state_v2(
    q, k, v, beta, M, scale=None, C=64, DVB=None, BI=None, return_us=False
):
    B, H, T, D = q.shape
    DV = v.shape[-1]
    E = D // M
    ...
    U = (
        torch.empty(B, H, T, DV, device=q.device, dtype=torch.float32)
        if return_us
        else o
    )
    _state_v2_kernel[(B * H,)](
        q,
        k,
        v,
        beta,
...
class _StateDelta(torch.autograd.Function):
    ...
    def backward(ctx, do):
        ...
        dq, dk, dv, db = spd_delta_state_bwd(
            q, k, v, beta, do, U, S, ctx.M, scale=ctx.scale, C=ctx.C
        )
        return (
            dq.to(q.dtype),
            dk.to(k.dtype),
...
def _state_bwd_kernel(
    ...
    for cc in range(NC):
        ...
        for i0 in range(0, E, BI):
            ...
            pS = tl.make_block_ptr(
                S + bs, (E2, DV), (DV, 1), (i0 * E, 0), (BI * E, DV), (1, 0)
            )
            tl.store(
                pS,
                (
                    tl.load(pS, boundary_check=(0, 1))
                    - tl.dot(tl.trans(pK.to(tl.bfloat16)), b_U.to(tl.bfloat16))
                ).to(S.dtype.element_ty),
                boundary_check=(0, 1),
            )
        ...
        dU = tl.zeros([C, DV], dtype=tl.float32)
        for i0 in range(0, E, BI):
            pK = tl.zeros([C, BI * E], dtype=tl.float32)
            for g in range(M):
                kg = (
        ...
        dLqk = tl.where(
            o_row[:, None] >= o_row[None, :],
            tl.dot(b_do.to(tl.bfloat16), tl.trans(b_U).to(tl.bfloat16)),
            0.0,
        )
        ...
        for i0 in range(0, E, BI):
            ...
            dPq = tl.dot(
                b_do.to(tl.bfloat16), tl.trans(S_blk).to(tl.bfloat16)
            ) + tl.dot(dLqk.to(tl.bfloat16), pK.to(tl.bfloat16))
            dPk = (
                tl.dot(b_U.to(tl.bfloat16), tl.trans(dS_blk).to(tl.bfloat16))
                + tl.dot(dE.to(tl.bfloat16), tl.trans(S_blk).to(tl.bfloat16))
                + tl.dot(dAkk_s.to(tl.bfloat16), pK.to(tl.bfloat16))
                + tl.dot(tl.trans(dLqk).to(tl.bfloat16), pQ.to(tl.bfloat16))
            )
            dPq3 = tl.reshape(dPq, (C, BI, E))
            dPk3 = tl.reshape(dPk, (C, BI, E))
            for g in range(M):
                qg = tl.load(
                    tl.make_block_ptr(
                        q + bqk, (T, D), (D, 1), (c0, g * E), (C, E), (1, 0)
