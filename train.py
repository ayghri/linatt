...
def main(cfg: DictConfig) -> None:
    ...
    # fla CausalLM heads return a per-microbatch MEAN cross-entropy and ignore
    # the `num_items_in_batch` kwarg. Their forward(**kwargs) signature makes HF
    # Trainer set model_accepts_loss_kwargs=True, which assumes the loss is a
    # GLOBALLY token-normalized SUM -- so the Trainer skips divide-by-grad_accum
    # and multiplies the loss by num_processes (average_tokens_across_devices).
    # On the 4-GPU x grad_accum=2 run that inflated BOTH the logged loss and
    # the backward gradients by 8x = num_processes(4) x skipped-grad-accum-
    # normalization(2): logged train_loss ~21 instead of the true ~2.6, and
    # max_grad_norm=1.0 clipping stayed active all run -> late-step updates
    # renormalized up, stalling convergence. Declaring the model does NOT
    # accept loss kwargs restores the correct mean-of-means path.
    ...
    if is_main:
        ...
        logger.info(
            f"Params: trainable={n_train:,} / total={n_all:,} ({n_train / n_all:.2%})"
        )
    ...
    args = TrainingArguments(
        ...
        dataloader_persistent_workers=cfg.train.get(
